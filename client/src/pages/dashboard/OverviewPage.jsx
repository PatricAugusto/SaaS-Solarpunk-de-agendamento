  ...
  return (
    <>
      ...
      <GlassPanel>
        ...
        {nextBooking ? (
          <div>
            <strong>{nextBooking.guest_name}</strong> —{" "}
            {nextBooking.event_title}
            <div
              className="font-mono"
              style={{ color: "#3DFDFF", marginTop: 6 }}
            >
              {format(
                new Date(nextBooking.start_time),
                "dd/MM/yyyy 'às' HH:mm",
                { locale: ptBR },
              )}
            </div>
      ...
      >
        ...
        <Link to="/dashboard/agendar">
          <NeonButton $variant="green">Agendar horário</NeonButton>
        </Link>
